...
Example Usage:
    python adware.py --data_dir=/tmp/adware --read_attn=True --write_attn=True
...
seq_inputs = [tf.placeholder(tf.float32, shape=(batch_size, img_size)) for _ in range(T)]
...
def linear(x, output_dim):
    """ affine transformation Wx+b assumes x.shape = (batch_size, num_features) """
...
# ll = tf.ones(shape=(batch_size, img_size))
...
for t in range(T):
    ...
    # print(tf.equal(x, ll, name="vae_sampling/comparison"))
    # ll = x
